...
<script>
import { searchOptions } from '@base/module_oms/after_sale_order'
import { getOmsSearchType, getCustorm } from '@/axios/module_oms/after_sale_order'
...
export default {
  ...
  async created() {
    ...
    this.$set(this.searchOptions[7], 'options', res1.data.data)
    // 客商信息全部数据源
    this.optionsCopy = res1.data.data
    // 获取搜索类型
    const res = await getOmsSearchType()
    const lest = res.data.data
    this.$set(this.searchOptions[5], 'options', lest.returnOrderType)
    this.$set(this.searchOptions[4], 'options', lest.bizType)
    this.$set(this.searchOptions[6], 'options', lest.returnOrderStatus)
  },
  async mounted() {

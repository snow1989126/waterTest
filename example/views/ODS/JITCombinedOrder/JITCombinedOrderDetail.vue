<template>
  <el-container class="order-box ODSOrderDetailBox DeliveryDetail">
    ...
    <!-- 列表表格 -->
    <table-common v-loading="loading" v-if="flag" :height="340" :table-header="deliveryDetailtableHeader" :change-cell="[10]" :change-cell-color="'red'" :table-data="tableData" class="del" @getSortList="getSortMethod" />
    <!-- 分页器 -->
    <page-common
      ref="page"
      :api="`/bff/ods/api/v1/merge/dispatch/r/query_details`"
...
export default {
  ...
  data() {
    return {
      ...
      detailOptions: detailOptions, // 详情组件选项参数
      deliveryDetailtableHeader: deliveryDetailtableHeader, // 表格组件选项参数
      tableData: [], // 表格数组
      flag: false, // 控制表格显示变量
      baseText: {}, // 详情数据来源变量
      defaultExportOptions: { // 默认导出变量
        url: '/bff/ods/no_route/api/v1/merge/dispatch/r/export_list', // 导出地址
        data: {
          mergeDispatchNo: this.$route.params.id // 导出参数
        },
        method: 'post', // 导出方法
  ...
  methods: {
    ...
    exportWord() {
      ...
      this.$axios({
        ...
        link.href = url
        // 对下载的文件命名
        link.download = decodeURI(res.headers['content-disposition'].split('=')[1]) || '合发单导出数据表.csv'

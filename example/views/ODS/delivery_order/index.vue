  ...
  <div class="store-shelves-result">
    <HtmlPage ref="targetPage" :page-config="config">
      <template v-slot:table-btn="scope">
        <el-button type="text" @click="$router.push(`/ODS/DeliveryList/DeliveryDetail/${scope.row.billNo}/${scope.row.isMerge ? 1 : 0}`)">查看</el-button>
        <el-button type="text" @click="Send(scope.row.billNo)">导出</el-button>
        <el-button
          v-if="scope.row.isMerge"
          type="text"
          size="mini"
          @click="pushToJIT(scope.row)">查看合发单</el-button>
      </template>
      <template v-slot:table-billNo="scope">
        <router-link :to="{path: `/ODS/DeliveryList/DeliveryDetail/${scope.row.billNo}`}" style="border-bottom: 1px solid">{{ scope.row.billNo }}</router-link>
      </template>
      <template v-slot:table-orderNo="scope">
        <router-link
          v-if="(scope.row['bizType'] === 'B2B_RECEIVE' || scope.row['bizType'] === 'B2B_DISPATCH')"
          :to="{path: `/TradingPlatform/List/ListDetail/${scope.row.orderNo}`}"
...
import { config } from '@base/module_ods/delivery_order'
import { getCurrentTime } from '@/utils/globalFunction'
import { getMessage, getCustorm, getwarehouseList, getwarehouseType, ExportExcel } from '@/axios/module_ods/delivery_order'
export default {
  ...
  async created() {
    ...
    const StaArr = this.MessageArr.filter(item => item.typeCode === 'dispatch_bill_status')[0].dictCodes
    const Bizarr = this.MessageArr.filter(item => item.typeCode === 'dispatch_bill_biz_type')[0].dictCodes
  ...
  methods: {
    ...
    pushToJIT(row) {
      sessionStorage.setItem('JITBillNo', row.billNo)
      this.$router.push(`/ODS/JITCombinedOrder`)
    }
  }
}
</script>
